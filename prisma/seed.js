...
export const customFaker = new Faker({ locale: [fr, en, base] })

async function main() {
	console.log("Seeding database...")

	for (let i = 0; i < 10; i++) {
		await prisma.tag.create({ data: { name: customFaker.company.name() } })
	}
	...
	for (let i = 0; i < 20; i++) {
		await prisma.ingredient.create({
			data: { name: customFaker.food.adjective(), unit_measure: "g" },
		})
	}

	for (let i = 0; i < 10; i++) {
		await prisma.category.create({
			data: { name: customFaker.animal.cat() },
		})
	}

	for (let i = 0; i < 10; i++) {
		await prisma.tool.create({
			data: { name: customFaker.animal.dog(), recipes: {} },
		})
	}
