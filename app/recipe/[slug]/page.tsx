...
const RecipeDetail = ({params}: TRecipeProps) => {
    ...
    useEffect(() => {
        const loadSuggestions = async () => {
            if (recipe) {
                try {
                    const data = await fetchSuggest(recipe.category.id, recipe.id);
                    console.log(data)
                    setSuggestions(data);
                } catch (error) {
        ...
        <div className={"flex flex-col gap-8 md:pt-4 md:px-12"}>
            <div className={"w-full rounded-lg bg-header md:h-[400px] flex flex-col md:flex-row md:justify-between"}>
                <div className="flex flex-col p-4 w-full md:w-1/2 items-center justify-center gap-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            className={"text-white text-sm font-medium flex gap-2 items-center rounded-full bg-secondary px-4 py-2 group"}>
                        ...
                        <button
                            ...
                            <Heart fill={favorites.some((recp) => recp.id === recipe?.id) ? "red" : "none"}
            ...
            <div className="w-full flex flex-col md:flex-row gap-8">
                <div className="md:w-1/2">
                    ...
                    <p className="font-medium text-foreground text-lg leading-relaxed text-justify">{recipe?.instructions}</p>
                </div>
                <div className="md:w-1/2 h-full">
                    ...
                    <TabGroup className={"border border-slate-600/50 rounded-md"}>
                        <TabList className="flex p-2 bg-header font-bold rounded-md duration-300">
                            <Tab
                                className={"rounded-md py-2 px-4 focus:outline-none data-[selected]:bg-secondary data-[hover]:bg-secondary/50 data-[selected]:data-[hover]:bg-secondary/10 data-[focus]:outline-1 data-[focus]:outline-white"}>Ingredients</Tab>
                            <Tab
                                className={"rounded-md py-2 px-4 focus:outline-none data-[selected]:bg-secondary data-[hover]:bg-secondary/50 data-[selected]:data-[hover]:bg-secondary/10 data-[focus]:outline-1 data-[focus]:outline-white"}>Tools</Tab>
                        </TabList>
                            ...
                            <TabPanel className={"flex gap-4 flex-wrap text-foreground text-lg font-medium"}>
                                {recipe?.ingredients.map((ingredient) => (
                                    <div key={ingredient.id}>
                                        <Image src={ingredient.ingredient.image} alt={ingredient.ingredient.name}
                                               width={100}
                                               height={100}
                                               className={"rounded-lg w-[100px] h-[100px] md:h-[150px] md:w-[150px] object-cover"}/>
                                        <h3 className={'font-bold text-md md:text-xl text-center'}>{ingredient.ingredient.name}</h3>
                                        <p className={'text-sm md:text-md text-gray-400 text-center'}>{ingredient.quantity} {ingredient.unit_measure}</p>
                                    </div>
                                ))}
                            </TabPanel>
                            <TabPanel
                                className={"grid grid-cols-2 md:flex md:items-center gap-4 md:flex-wrap text-foreground text-lg font-medium"}>
                                {recipe?.tools.map((tool) => (
                                    <div key={tool.id}
                                         className="flex flex-col justify-center items-center md:block text-foreground text-lg font-medium">
                                        <Image src={tool.tool.image} alt={tool.tool.name} width={100} height={100}
                                               className={"rounded-lg w-[100px] h-[100px] md:h-[150px] md:w-[150px] object-cover"}/>
                                        <h3 className={'font-bold text-md md:text-xl text-center'}>{tool.tool.name}</h3>
                                    </div>
                                ))}
                            </TabPanel>
                        </TabPanels>
                ...
                <Swiper {...swiperParams}>
                    {recipe?.steps && recipe?.steps.map((step) => (
                        ...
                        >
                            ...
                            <p className={"text-foreground text-lg leading-relaxed"}>{step.content}</p>
                        </SwiperSlide>
                    ))}
                </Swiper>
            </div>
                        ...
                        <div>
                            ...
                            <form action="">
                                ...
                                <textarea
                                    ...
                                    draggable={false} placeholder="Write your comment here..."/>
                                    <button
            ...
            <div>
                ...
                <div className="flex flex-col gap-4">
                    {suggestions && suggestions.map((suggestion) => (
                        <RecipeCard key={suggestion.id} recipe={suggestion} detailed={true}/>
                    ))}
                </div>
            </div>
        </div>
